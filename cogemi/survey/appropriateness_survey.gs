// cogemi/survey/appropriateness_survey.gs
...
//   timestamp | prolific_id | scenario_id | response | age | gender | country | language | assigned_group
...
// ---------------------------------------------------------------------------
// Scenario pool
// Each scenario: { id, en, fr }
// ID format: sa_{action}_{setting}  — matches COGEMI ContextLearner convention
// ---------------------------------------------------------------------------
...
// Attention check items (fixed positions in the sequence)
var ATTENTION_CHECKS = [
  { id: "sa_check_appropriate",   en: "Someone holds the door open for the person directly behind them.", fr: "Quelqu'un tient la porte à la personne juste derrière lui.", expected: "Appropriate" },
...
function doGet() {
  return HtmlService.createHtmlOutputFromFile("appropriateness_survey")
    .setTitle("Social Appropriateness Study")
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// ---------------------------------------------------------------------------
// Called by the client to get the shuffled scenario list for this session
// ---------------------------------------------------------------------------

function getScenariosForSession(language) {
  var pool = SCENARIOS.slice(); // copy
  // Fisher-Yates shuffle
  for (var i = pool.length - 1; i > 0; i--) {
    var j = Math.floor(Math.random() * (i + 1));
    var tmp = pool[i]; pool[i] = pool[j]; pool[j] = tmp;
  }
  // Insert attention checks at fixed positions (0-indexed: positions 7 and 17)
  pool.splice(7, 0, ATTENTION_CHECKS[0]);
  pool.splice(17, 0, ATTENTION_CHECKS[1]);

  // Return only the text for the chosen language plus the id
...
// ---------------------------------------------------------------------------
// Called by the client to submit a single response
// ---------------------------------------------------------------------------

function submitResponse(prolificId, scenarioId, response, age, gender, country, language) {
  if (!prolificId || !scenarioId || !response) return { ok: false, error: "Missing required fields" };

  var sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName(SHEET_NAME);
  if (!sheet) return { ok: false, error: "Sheet not found" };

  var timestamp = new Date();
  sheet.appendRow([timestamp, prolificId, scenarioId, response, age, gender, country, language]);
  return { ok: true };
}

// ---------------------------------------------------------------------------
// Utility: export responses in COGEMI SurveyResponse format (for debugging)
// Returns first 1000 rows as array of {participant_id, scenario_id, response}
// ---------------------------------------------------------------------------
